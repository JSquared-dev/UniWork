...
void processPacket(struct lanPacket_s *packet, struct threadData_s *threadData) {
	...
	if (packet->destination == userID) {
		...
		else if (packet->packetType == LOGIN_PACKET && packet->source == userID && threadData->programState != LOGIN && threadData->programState != LOGIN_PEND) {
			/* if this instance is logged in, and we receive a login packet, someone is trying to login as our user.
			 * decline their login request with a NAK packet and prevent the LOGIN packet going any further */
